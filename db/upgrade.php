...
function xmldb_ratingallocate_upgrade($oldversion) {
    ...
    if ($oldversion < 2015041300) {
        try {
            ...
            $field2 = new xmldb_field('algorithmstarttime', XMLDB_TYPE_INTEGER, '10', null, false,
                null, null, 'runalgorithmbycron');
            ...
            foreach ($results as $singleresult) {
                ...
                if (count($allocations) === 0) {
                    $singleresult->algorithmstatus = \mod_ratingallocate\algorithm_status::NOTSTARTED;
                } else {
                    $singleresult->algorithmstatus = \mod_ratingallocate\algorithm_status::FINISHED;
                }
                $DB->update_record('ratingallocate', $singleresult);
            }
